...
Fix: COPY file1 /app → destination /app add kiya

apt install python3 failed

apt install python3 -y
Fix: apt update add kiya before install

RUN apt update && apt install -y python3
requirements.txt not found

Could not open requirements file: [Errno 2] No such file or directory
Fix: Dockerfile me COPY requirements.txt /app add kiya

Container run failed (port already allocated)

Bind for 0.0.0.0:5000 failed: port is already allocated
Fix: Existing container stop & remove karo

docker ps
docker stop <container_id>
docker rm <container_id>
Multiple containers ke liye host port alag use karo: -p 5001:5000

Flask app not accessible externally

Fix: Flask app run karo with host="0.0.0.0"
